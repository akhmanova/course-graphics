...
#pragma endregion
	...
	private: System::Void MyForm_KeyDown(System::Object^  sender, System::Windows::Forms::KeyEventArgs^  e) {
		...
		case Keys::Z:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = scale(1.1) * T; //увеличение изображения относительно текущего центра в 1.1 раза
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
			break;
		case Keys::X:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = scale(0.9) * T; //уменьшение изображения относительно текущего центра в 1.1 раза
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
			break;
		case Keys::I:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = scale(1.1, 1) * T; //растяжение изображения относительно текущего центра по горизонтали в 1.1 раза
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
			break;
		case Keys::K:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = scale(0.9, 1) * T; //сжатие изображения относительно текущего центра по горизонтали в 1.1 раза
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
			break;
		case Keys::O:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
		...
		case Keys::L:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = scale(1, 0.9) * T; //сжатие изображения относительно текущего центра по горизонтали в 1.1 раза
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
			break;
		case Keys::U:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = mirrorX() * T; // отражение по оси X относильно центра
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
			break;
		case Keys::J:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = mirrorY() * T; // отражение по оси Y относильно центра
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
			break;
		default:
			break;
